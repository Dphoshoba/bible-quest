...
function App() {
  ...
  // Separate useEffect for audio initialization
  useEffect(() => {
    ...
    const initAudio = () => {
      try {
        ...
        if (bgMusic) {
          bgMusic.src = '/sounds/general.mp3';
          bgMusic.volume = 0.2;
        ...
        if (whooshAudio) {
          whooshAudio.src = '/sounds/click.mp3';
      ...
      <CharacterContext.Provider value={{ characters, isPremiumUser }}>
        <Router>
          <div className={`app-wrapper${darkMode ? ' dark-mode' : ''}`} style={{ 
            ...
            <audio id="bg-music" preload="none" muted></audio>
            <audio id="whoosh-audio" preload="none"></audio>
